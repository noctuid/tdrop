...
If you are using `Hyprland` then [hdrop](https://github.com/Schweber/hdrop/tree/main) can emulate the main feature of `tdrop`, namely running, showing and hiding a given program depending on its current state.
